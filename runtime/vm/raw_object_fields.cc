...
OffsetsTable::OffsetsTable(Zone* zone) : cached_offsets_(zone) {
  for (intptr_t i = 0; offsets_table[i].class_name != nullptr; ++i) {
    OffsetsTableEntry entry = offsets_table[i];
    cached_offsets_.Insert(
        {{entry.class_name, entry.offset}, entry.field_name});
  }
}

const char* OffsetsTable::FieldNameForOffset(const char* class_name,
                                             intptr_t offset) {
  return cached_offsets_.LookupValue({class_name, offset});
}

#define DEFINE_OFFSETS_TABLE_ENTRY(class_name, field_name)                     \
  {#class_name, #field_name, OFFSET_OF(Raw##class_name, field_name)},
